    ...
    fetch(encodedFileName)
        ...
        .then(markdown => {
            ...
            if (typeof marked === 'undefined') {
                ...
                contentDiv.innerHTML = '<div class="error">Markdown parser not available</div>';
                return;
            }
            ...
            renderer.heading = function(text, level) {
                // Make sure text is a string and handle any object/array cases
                if (typeof text !== 'string') {
                    console.warn('Heading text is not a string:', text);
                    // If text is an object or array, try to stringify it
                    try {
            ...
            try {
                ...
                console.log('Parsed HTML type:', typeof html);
                console.log('Parsed HTML preview:', html.substring(0, 200));
                
                // Ensure we have a string
                if (typeof html !== 'string') {
                    console.error('marked returned non-string:', html);
                    html = String(html);
                }
            } catch (error) {
                console.error('Error parsing markdown:', error);
                html = '<div class="error">Error parsing markdown content</div>';
            }
