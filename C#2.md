...
### Method Overloading
```csharp
// Multiple methods with same name, different parameters (Method Overloading)
...
### **Named Parameters**
```csharp
// Method with default parameter and usage of named parameters
...
```csharp
// Method using ref parameters (must be initialized before passing)
...
// Method using out parameters (doesn't need to be initialized)
...
### **Params Keyword**
```csharp
// Method using params to accept variable number of arguments
...
// Usage examples
int result1 = Sum(1, 2, 3, 4, 5);    // Sum of 5 numbers
int result2 = Sum(10, 20);           // Sum of 2 numbers
int[] array = { 1, 2, 3 };
int result3 = Sum(array);             // Sum using an array
...
// Calling overridden and base methods
foreach (Animal animal in animals)
{
    animal.MakeSound();  // Calls the appropriate override
    animal.Sleep();      // Calls the base method
}

// Casting and type checking to call derived-class-specific methods
foreach (Animal animal in animals)
{
...
// Query syntax: select even numbers
var evenNumbers = from num in numbers
                  where num % 2 == 0
                  select num;

// Method syntax: select odd numbers, squares, and sort descending
...
// Select adults, order by name, and project names
var adults = people
    .Where(p => p.Age >= 18)
    .OrderBy(p => p.Name)
    .Select(p => p.Name);
...
// Async method to download content from a URL
...
var customerResult = GetCustomer(); // Type not obvious, consider explicit type

// Dispose resources properly using 'using'
...
### **4. Banking System with Design Patterns**
```csharp
using System;
using System.Collections.Generic;
...
// Account class implementing Strategy and Observer
...
- **Godot** (C# support) – Open-source game engine supporting C# for scripting
