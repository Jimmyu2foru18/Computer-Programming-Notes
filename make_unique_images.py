...
Fixes duplicate images within the same file by replacing them individually.
"""
...
# All verified working Unsplash photo IDs (consolidated)
...
# Collect all image occurrences with their positions
all_images = []
...
# Apply changes - process file by file to handle duplicates correctly
print("\nApplying changes...")
files_modified = 0
...
for img in all_images:
    images_by_file[img['filepath']].append(img)

for filepath, images in images_by_file.items():
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Sort images by start position (descending) to replace from end to start
    # This prevents position shifting issues
    images_sorted = sorted(images, key=lambda x: x['start'], reverse=True)
    ...
    for img in images_sorted:
        ...
        # Find the specific occurrence at this position
        if content[img['start']:img['end']] == img['match'].group(0):
            # Replace just this occurrence
            before = content[:img['start']]
            after = content[img['end']:]
            content = before + img['match'].group(0).replace(old_src, new_src, 1) + after
            modified = True
        else:
            print(f"WARNING: Position mismatch in {os.path.basename(filepath)}")
